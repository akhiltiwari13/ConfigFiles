...
# --- build parallelism cap (low-memory hosts) --------------------------------
# This laptop has 16 GiB RAM but 22 logical CPUs, so an unbounded build (bare
# `ninja` = nproc+2 = 24 jobs, Conan presets = 22) will exhaust memory. The
# "cap at 6 cores" rule was documented in ~/.claude/CLAUDE.md but never actually
# exported anywhere; this enforces it.
#
# Guarded on total RAM so it is a no-op on bigger machines. The explicit
# readability test matters: on macOS /proc/meminfo does not exist, and an empty
# awk result would be read by bash arithmetic as 0 (<= 20) and wrongly apply.
#
# Deliberately NOT exporting SQPL_COMPILE_CORES: quant-research's setup_ib.sh
# falls back to -j4 without it, and pqr has the heaviest translation units in the
# tree (15-17 MB .o). Raising pqr 4 -> 6 would be a regression. MAKEFLAGS and
# CMAKE_BUILD_PARALLEL_LEVEL already cap the other repos.
#
# For a hard *memory* ceiling on a single build, use `membuild` (see
# omarchy-overrides/.config/bin/membuild).
if [[ -r /proc/meminfo ]] &&
