...
- **`omarchy-font-set` breaks foot's stow symlink**: it `sed -i`s `~/.config/foot/foot.ini`, and GNU `sed -i` renames over the target, replacing the symlink with a real file. It also hardcodes `size=9` and drops `:weight=semibold`, undoing the ghostty font match. Any in-place editor does the same — one did on 2026-09-01. Recover with `stow -n -v --adopt foot` then `stow --adopt foot`, which keeps your value and re-tracks it
